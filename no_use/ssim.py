...
def ssim(X, Y, window, data_range: float, use_padding: bool=False, device='cpu'):
    '''
...
def ms_ssim(X, Y, window, data_range: float, weights, use_padding: bool=False, eps: float=1e-8, device='cpu'):
    '''
    ...
    for i in range(levels):
        ss, cs = ssim(X, Y, window=window, data_range=data_range, use_padding=use_padding, device=device)
...
if __name__=='__main__':
    ...
    loss_fn = SSIM(data_range=1.)# SSIM
    # loss_fn = MS_SSIM(data_range=1., levels=3)# muti scale SSIM
    loss_fn.to(device)
    B=4
    
    
    # you should try by using your images
    x=torch.rand([B, 3, 128,128]).to(device)
    
    y_1 = _gaussian_filter(x, create_window(7,1,3).to(device), True) #blur input, looks bad, but MSE loss is low.
    
    y_2 = x*0.8 # reduce inputs brightness, looks like the same, but MSE loss is high.
    
    
    print('reduce brightness with MSE: ', (x-y_1).square().mean())
    print('reduce brightness with SSIM: ', 1 - loss_fn(x,y_1))
    
    print('reduce brightness with MSE: ', (x-y_2).square().mean())
    print('reduce brightness with SSIM: ', 1 - loss_fn(x,y_2))
